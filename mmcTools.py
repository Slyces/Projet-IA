...
def signature(fonction):
    ...
    def enveloppe(*args, **kargs):
        for name, arg in (list(zip(arg_spec.args, args)) +
            ...
            "expected argument {0} of type {1} got {2} in function '{3}'".format(name, annotations[name], type(arg),
                                                                                 fonction.__name__))
        _r = fonction(*args, **kargs)
        if has_return:
            assert isinstance(_r, annotations["return"]), (
            "{2}: expected return of {0} got {1}".format(annotations["return"], type(_r), fonction.__name__))
        return _r
    ...
    def make_property(name, att):
        ...
        def setMe(self, val):
            ...
            if _old is None or not att.lock:
                if att.propriete(name, val):
                    setattr(self, _prive, val)
                else:
                    raise ValueError("cant set {} to {}".format(name, val))
...
if __name__ == "__main__":
    ...
    @signature
    def gun(x: int, y: float = 1.2) -> int:
        y ** x  # genere une erreur
