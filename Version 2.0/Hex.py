...
class Game(tk.Tk):
    ...
    def play(self, i, j, color=None):
        color = [BLANC, NOIR][self.player] if color == None else color
    ...
    def on_click(self, ev, color= None):
        color = [BLANC,NOIR][self.player] if not color else color
        for (i, j), hex in self.__hexagons.items():
    ...
    def display(self):
        ...
        self.canvas.create_image(self.width/2, self.width/1.8, image=self.__tokens[self.player])
